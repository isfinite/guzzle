...
Guzzle.prototype.run = function() {
	inquirer.prompt(prompts, function(data) {
		...
		this.writeScaffold('package', 'json', function() {
			...
			var devDependencies = ['gulp', 'gulp-clean', 'gulp-concat', 'gulp-minify-css', 'gulp-rename', 'gulp-uglify'];

			if (data.use_bower) {
				devDependencies.push('gulp-bower-files');
			}

			if (data.use_testing) {
				devDependencies.push('mocha');
				devDependencies.push('should');
				devDependencies.push('gulp-mocha');
			}

			npm.load(npmConfig, function(err, npm) {
